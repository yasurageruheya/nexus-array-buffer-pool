...
export const test = ()=> {
	...
	it("await pool.free().detached.done で正しく待機できる", async () => {
	...
	it("複数の pool.free().detached.done を一括で await 出来る", async () => {
		const {nexus} = await getNexus({useSpy:false});
	...
	it("1つの pool.free().detached.done.then() に複数のコールバック（ユーザー関数）が捧げられても、正常に待機して解決できる(arrayBuffer.transfer() に対応していない場合のみテスト)", async () => {
		...
		const results = [];
		const wait = (id:string) => status.detached.done.then(() => {
			results.push(id);
		});
		...
		expect(internalStatus.onSettles.length).toBe(3);
		...
		expect(buffer.byteLength).toBe(0);
		expect(internalStatus.onDetaches).toBe(null);
		expect(internalStatus.onSettles).toBe(null);
		expect(internal.pool[slabIndex].size).toBe(1);
		expect(results).toEqual(["A", "B", "C"]);
		expect(results.length).toBe(3);
	});

	it("1つの pool.free().detached.done に対して複数の非同期コンテキストで同時に await されても、全てが正常に再開され、全てが同時に解決される", async () => {
		const {nexus} = await getNexus({useSpy:false});
		const buffer = new ArrayBuffer(1024);
		const status = nexus.free(buffer);
		...
		const createAwaiter = async (id: string) => {
			const internalStatus = internal.internalCleaningStatuses.get(status.id);
			await status.detached.done; // ここで Watcher の then が呼ばれる
			return {id, isNull: internalStatus.onSettles === null}; //どの時点でも onSettles が null であれば、全て同時に終わっている事になる
		};
